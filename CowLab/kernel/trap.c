    ...
    while(1)
    {
      uint64 pgvaddr=r_stval();
       ...
       if((*pte)&PTE_RSW_COW_W)
        { 
          //  printf("!COW_R/COW_W/R/W:%d %d %d %d\n",!!(*pte&PTE_RSW_COW_R),!!(PTE_FLAGS(*pte)&PTE_RSW_COW_W),!!(PTE_FLAGS(*pte)&PTE_R),!!(PTE_FLAGS(*pte)&PTE_RSW_COW_W));
         ...
         break;
          // uvmunmap(p->pagetable,PGROUNDDOWN(pgvaddr),1,0);
          // mappages(p->pagetable,pgvaddr,PGSIZE,newpgpa,(PTE_FLAGS(*pte)&(!PTE_RSW_COW_W))|PTE_W);
        }
        else{
