...
mmap_test(void)
{
  ...
  if ((fd = open(f, O_RDONLY)) == -1)
    err("open");
  p = mmap(0, PGSIZE*3, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);//question that here above that with ronly and prot write can be succeeded
  if (p != MAP_FAILED)
