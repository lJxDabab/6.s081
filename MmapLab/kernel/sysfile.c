...
sys_mmap(void)
{
  ...
  if(!addr)
  {
      ...
      p->pf[i].prot=port;
      p->pf[i].flag=flag;
      p->pf[i].length=PGROUNDUP(len);
      p->pf[i].addr=tranpoffset-len;
      p->pf[i].f=p->ofile[fd];
      if((!p->pf[i].f->writable)&&(port&PROT_WRITE)&&(flag&MAP_SHARED))
    {
      p->pf[i].pos=0;
       p->pf[i].valid=0;
      return 0xffffffffffffffff;
...
sys_munmap(void)
{
  ...
  struct proc *p=myproc();
   for(i=0;i<VMAXSZ-1;i++)
      {
        if(p->pf[i].addr<=addr&&p->pf[i].addr+p->pf[i].length >addr)
        {
          break;
        }
      }
  ...
  if(p->pf[i].flag==MAP_PRIVATE)
  { 
    if(*pte&PTE_V)
      uvmunmap(p->pagetable,addr,len/PGSIZE,0);
  }
  else if(p->pf[i].flag==MAP_SHARED)
  {
     if(*pte&PTE_V)
     {
